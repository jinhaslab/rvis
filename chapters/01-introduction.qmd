...
**저장소**: [https://github.com/jinhaslab/rvis](https://github.com/jinhaslab/rvis)

**실습 스크립트**:
- [code/setup.R](https://github.com/jinhaslab/rvis/blob/main/code/setup.R) - 패키지 설치
- [code/data-simulation.R](https://github.com/jinhaslab/rvis/blob/main/code/data-simulation.R) - 데이터 생성
...
- **macOS 사용자**: XQuartz 설치 권장 (일부 그래픽 패키지에 필요)
  - 다운로드: [https://www.xquartz.org/](https://www.xquartz.org/)
...
**다운로드**: [https://posit.co/download/rstudio-desktop/](https://posit.co/download/rstudio-desktop/) (무료)
...
**직접 링크:**
- [setup.R](https://raw.githubusercontent.com/jinhaslab/rvis/main/code/setup.R)
- [data-simulation.R](https://raw.githubusercontent.com/jinhaslab/rvis/main/code/data-simulation.R)
...
패키지 설치는 **최초 1회만** 10-20분 정도 걸립니다.
한 번 설치하면 다음부터는 `library()`로 빠르게 로드할 수 있습니다.
:::
...
**두 가지 파이프:**
- `%>%`: magrittr 패키지 파이프 (tidyverse 기본)
- `|>`: R 4.1+ 기본 파이프 (native pipe)

**차이점**: 대부분의 경우 동일하게 작동. 이 책에서는 `%>%` 사용.

**단축키**:
- RStudio: Ctrl+Shift+M (Windows/Linux), Cmd+Shift+M (Mac)
- 설정에서 `|>` 대신 `%>%` 생성하도록 변경 가능
:::
...
**요구사항:**
1. x축: 꽃받침 길이 (Sepal.Length)
2. y축: 꽃받침 너비 (Sepal.Width)
3. 제목: "붓꽃 꽃받침 크기"
4. 축 레이블에 단위 표시 (cm)
5. 깔끔한 테마 적용
...
**요구사항:**
- 3개 종(setosa, versicolor, virginica)을 서로 다른 색으로 표시
- 범례 추가
...
1. 30세 이상만 필터링
2. 성별(gender)로 그룹화
3. BMI와 혈당(glucose)의 관계를 성별로 구분하여 시각화
...
✅ **1.1 왜 R인가?**
- R의 보건학 연구에서의 위치와 장점
- R vs. SPSS, SAS, Stata, Python 비교
- 재현 가능한 연구의 중요성

✅ **1.2 개발 환경 설정**
- R 4.3+ 및 RStudio 설치
- RStudio 인터페이스 이해 (4개 패널)
- 프로젝트 폴더 구조 생성
- GitHub에서 실습 스크립트 다운로드
- 40개 이상 패키지 설치 (tidyverse, ggplot2 등)
- 실습 데이터 6개 생성

✅ **1.3 R 기초 복습**
- 벡터, 데이터프레임 이해
- tidyverse 생태계 소개
- dplyr의 5가지 핵심 동사 (filter, select, mutate, summarize, arrange)
- 파이프 연산자 (%>%) 활용

✅ **1.4 ggplot2: 그래픽 문법**
- Grammar of Graphics 개념
- ggplot2의 7가지 구성 요소
- 첫 번째 산점도 만들기
- 레이어 구조로 그래프 쌓기
...
✅ **1.6 실습**
- 5가지 연습 문제 해결
- iris, mtcars, health_survey 데이터 시각화
...
### 공식 문서
- **R 공식 사이트**: [https://www.r-project.org/](https://www.r-project.org/)
- **ggplot2 공식 문서**: [https://ggplot2.tidyverse.org/](https://ggplot2.tidyverse.org/)
- **tidyverse**: [https://www.tidyverse.org/](https://www.tidyverse.org/)
- **RStudio IDE**: [https://posit.co/](https://posit.co/)
...
- **Applied Epi**: [https://www.appliedepi.org/](https://www.appliedepi.org/)
...
### 이 책
- **교재 GitHub**: [https://github.com/jinhaslab/rvis](https://github.com/jinhaslab/rvis)
- **온라인 북**: [https://jinhaslab.github.io/rvis/](https://jinhaslab.github.io/rvis/)
- **문의**: jinhaslab@gmail.com
:::
...
**초보자를 위한 팁:**
1. 매일 30분씩 꾸준히 연습하세요
2. 에러를 두려워하지 마세요 (에러는 배움의 기회!)
3. 예제 코드를 직접 타이핑하세요 (복사/붙여넣기 ❌)
4. 작은 변화를 주며 실험하세요
5. 커뮤니티에 질문하는 것을 망설이지 마세요

**중급자를 위한 팁:**
1. 자신만의 프로젝트를 시작하세요
2. GitHub에 코드를 공유하세요
3. 다른 사람의 코드를 읽고 배우세요
4. 최신 패키지와 기법을 탐색하세요
5. R 블로그를 작성하거나 발표를 해보세요
