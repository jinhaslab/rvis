...
**예제 2: survival 패키지의 lung 데이터**

```{r}
#| echo: true
#| eval: true

# ⚠️ 중요: 패키지를 먼저 불러와야 data() 작동!
library(survival)   # 1단계: 패키지 로드
data(lung)          # 2단계: lung 데이터 불러오기
...
**팁:**
- `data(lung, package = "survival")` 처럼 `package` 인자를 사용하면 library() 없이도 불러올 수 있지만, 실전에서는 `library()` 사용을 권장합니다.
:::
